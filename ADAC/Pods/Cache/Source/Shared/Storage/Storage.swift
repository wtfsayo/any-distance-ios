...
public final class Storage<Key: Hashable, Value> {
  ...
  public init(hybridStorage: HybridStorage<Key, Value>) {
    ...
    self.syncStorage = SyncStorage(
      ...
      serialQueue: DispatchQueue(label: "Cache.SyncStorage.SerialQueue", qos: .userInitiated)
    )
    self.asyncStorage = AsyncStorage(
      storage: hybridStorage,
      serialQueue: DispatchQueue(label: "Cache.AsyncStorage.SerialQueue", qos: .userInitiated)
    )
  }
