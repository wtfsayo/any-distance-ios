...
final class EditGoalViewController: UIViewController {
    ...
    @IBOutlet weak var freeGoalsRemainingLabel: UILabel!
    ...
    func setup() {
        ...
        if mode == .createGoal {
            ...
            bottomButton.setTitle("Create Goal", for: .normal)
        }

        iAPManager.shared.$isSubscribed
            .receive(on: DispatchQueue.main)
            .sink { [weak self] _ in
                self?.updateFreeGoalsLabel()
            }.store(in: &subscribers)
    }

    private func updateFreeGoalsLabel() {
        if iAPManager.shared.hasSuperDistanceFeatures || mode == .editGoal {
            freeGoalsRemainingLabel.isHidden = true
            return
        }

        let n = NSUbiquitousKeyValueStore.default.freeGoalsRemaining
        freeGoalsRemainingLabel.text = "\(n) free goal\(n == 1 ? "" : "s") remaining"
    }
    ...
    @IBAction func setGoalTapped(_ sender: Any) {
        if NSUbiquitousKeyValueStore.default.freeGoalsRemaining == 0 && !iAPManager.shared.hasSuperDistanceFeatures && mode == .createGoal {
        ...
        if mode == .createGoal {
            NSUbiquitousKeyValueStore.default.freeGoalsRemaining -= 1
        }
...
fileprivate extension NSUbiquitousKeyValueStore {
    var freeGoalsRemaining: Int {
        get {
            if object(forKey: "freeGoalsRemaining") == nil {
                return 3
            }

            return integer(forKey: "freeGoalsRemaining")
        }

        set {
            set(newValue, forKey: "freeGoalsRemaining")
        }
    }
}
